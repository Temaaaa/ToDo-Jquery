...
var addTask = function() {
  ...
  if ( taskInput.val() == '' ) {
    alert('Вы не добавили задание!');
  } else {
      var listItem = $('<li><input type="checkbox"><label>' + taskInput.val() + '</label><input type="text"><button class="edit">Редактировать</button><button class="delete">Удалить</button></li>');
...
var markTask = function() {
  function checkEditState(task) {
    if ( task.hasClass('editMode') ) {  
      ...
      task.find('.edit').text('Редактировать');
    }
  }
